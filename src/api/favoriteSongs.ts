...
async function getFavoriteSongs() {
    const demo = await axios.get('https://musicplayernodejs-production.up.railway.app/mySongs')
    return demo.data
}
// async function getFavoriteSongs() {
...
async function updateFavoriteSongs(check: boolean, currentSong: any) {
    if (check) {
        await axios.post('https://musicplayernodejs-production.up.railway.app/removeLikeSong', currentSong)
    }
    else {
        await axios.post('https://musicplayernodejs-production.up.railway.app/addLikeSong', currentSong)
    }
}
